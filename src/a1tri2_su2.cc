...
void printElTypes(apf::Mesh* m)
{
  ...
  while ( (e = m->iterate(it)) )
  {
    ...
    if ( etype == apf::Mesh::TRIANGLE)
      tricount += 1;
    if ( etype == apf::Mesh::QUAD )
      quadcount += 1;
  }

  std::cout << "counted "  << tricount << " triangles, " << quadcount << "quads" << std::endl;

  std::cout << "maxpertx = " << maxpertx << std::endl;
  std::cout << "maxperty = " << maxperty << std::endl;
}
...
/*
  ...
  if ( fabs(pertx) > maxpertx)
    maxpertx = fabs(pertx);

  if ( fabs(perty) > maxperty)
    maxperty = fabs(perty);

  // apply a small amplitude sin wave
  coords[0] = coords[0] + pertx;
  coords[1] = coords[1] + perty;

  std::cout << "pertx = " << pertx << std::endl;
  std::cout << "perty = " << perty << std::endl;
*/
  /*
